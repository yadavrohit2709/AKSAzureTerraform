...
# This is the corrected version for Checkov security demo
...
resource "azurerm_storage_account" "aks_aksdemo_storage" {
  ...
  # checkov:skip=CKV2_AZURE_33: Private endpoint requires VNet setup in production
  ...
  # Queue properties with logging enabled (CKV2_AZURE_33)
  queue_properties {
    logging {
      delete                = true
      read                  = true
